...
def DE1(y,t,arr_parameters_IP):
    ...
    # Number of SPEN that bind to one Xist.  We let each chromosome be able to
    # recruit and bind to all SPEN.
    N_S = round(sT/XbsT) 
...
def results_of_3X(odes, SS_penalty, time_step, XbsT, sT):
    ...
    fitness = ((np.abs(x1b_SS-x1b_6000s) + np.abs(x2b_SS-x2b_6000s) + np.abs(x3b_SS-x3b_6000s) 
               ...
               + np.abs(XbsT/(1+x2b_SS-x1b_SS)) + 0.1*np.abs(sT - s1b_SS - s2b_SS - s3b_SS)) # penalize on free SPEN
                                                                                            # because we want SPEN to be depleted
    return fitness
...
def scorefxn1(arr_parameters, final_time, time_step, SS_penalty):
    ...
    fodes = odeint(DE2, IC, time, args=(arr_parameters_IP,) , rtol = 1e-8, atol=1e-8)
