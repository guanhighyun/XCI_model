...
% Initial conditions
act = 0; s1b = 0; x1b = 0; x1f = 0; s2b = 0; x2f = 0; x2b = 0;

% List of bound Xist initial conditions
s1b_IC = 0:2:194;
s2b_IC = 0:2:194;

x1bout = nan(numel(s1b_IC),numel(s2b_IC));
s1bout = nan(numel(s1b_IC),numel(s2b_IC));
x2bout = nan(numel(s1b_IC),numel(s2b_IC));
s2bout = nan(numel(s1b_IC),numel(s2b_IC));

% Perform scan
for i = 1:numel(s1b_IC)
    for j = 1:numel(s2b_IC)
        y0 = [act;x1f;x1b;s1b_IC(i);x2f;x2b;s2b_IC(j)];
...
regime = zeros(numel(s1b_IC), numel(s2b_IC));
...
set(gca,'fontsize',25)
axis square
caxis([0,5]); xticks(6:20:numel(s1b_IC)); xticklabels(s1b_IC(6:20:end));
yticks(6:20:numel(s1b_IC)); yticklabels(s1b_IC(6:20:end));
