...
% CHANGED: Start the sweep lower (0.5x) to ensure we don't miss the true lower boundary
new_sT_list = 750:10:1750; 

% Preallocate arrays to store BOTH min and max SPEN for each volume
spen_min = zeros(1, numel(V_list)); 
spen_max = zeros(1, numel(V_list)); 

for i = 1:numel(V_list)
    V = V_list(i);
    min_tolerated_sT = NaN; % NEW: Track the lower boundary
    max_tolerated_sT = NaN; % Track the upper boundary
    ...
    for j = 1:numel(new_sT_list)
        ...
        if is_2_of_4_state
            if ~found_2_of_4
                % CHANGED: This is the VERY FIRST time we hit the state!
                min_tolerated_sT = new_sT;
                found_2_of_4 = true; 
            end
            
            % We are in the 2-of-4 regime. Continuously update the max tolerated value.
            max_tolerated_sT = new_sT;
            
        elseif found_2_of_4
            % If we are NO LONGER in the state, but we WERE previously, 
            % we have found our upper boundary. Break the loop to save time.
            break; 
        end
    end
    
    % Store the lowest and highest tolerated SPEN abundance
    spen_min(i) = min_tolerated_sT;
    spen_max(i) = max_tolerated_sT;
    
    % Print progress to the console so you can monitor it
    fprintf('Volume: %.1f | Min sT: %.1f | Max sT: %.1f \n', V, min_tolerated_sT, max_tolerated_sT);
end

% Plotting the results
figure;
% CHANGED: Calculate the fold increase dynamically by dividing Max by the true Min
fold_tolerance = spen_max ./ spen_min;

plot(V_list, fold_tolerance, '-o', 'LineWidth', 4);
xlabel('Relative Nuclear Volume (V)');
ylabel('Maximum SPEN Increase (SPEN_{max} / SPEN_{min})');
title('Maximum Tolerated SPEN Abundance in Tetraploid Cells');
set(gca,'fontsize',15)
grid on; hold on; xlim([1,2]); ylim([1,2]);
