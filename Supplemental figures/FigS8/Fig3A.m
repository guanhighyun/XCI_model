...
base_sT = 1000;
sT_fold_list = 1:0.002:2;
V_fold_list = 1:0.002:2;
...
% Solve equations
x1bout = nan(numel(V_fold_list),numel(sT_fold_list));
x2bout = nan(numel(V_fold_list),numel(sT_fold_list));
x3bout = nan(numel(V_fold_list),numel(sT_fold_list));
x4bout = nan(numel(V_fold_list),numel(sT_fold_list));

for i = 1:numel(V_fold_list)
    for j = 1:numel(sT_fold_list)
        [t,y] = ode15s(@(tt,yy) ODE_model(tt,yy,V_fold_list(i),sT_fold_list(j)*base_sT),tspan,y0,options);
...
figure;
regime = zeros(numel(V_fold_list),numel(sT_fold_list));
...
imagesc(regime'); set(gca,'ydir','normal'); xlabel('Relative volume');
ylabel('Relative SPEN abundance S/S_{T}'); 
set(gca,'fontsize',20);
 xticks(1:100:numel(sT_fold_list))
 xticklabels(sT_fold_list(1:100:numel(sT_fold_list)))
 yticks(1:100:numel(sT_fold_list))
 yticklabels(V_fold_list(1:100:numel(sT_fold_list)))
axis square
