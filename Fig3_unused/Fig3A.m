...
k1_list = [0.002:0.0002:0.02];
k3_list = [0.005:0.001:0.1]; 
...
steady_state = ones(numel(k1_list),numel(k3_list));
...
for i = 1:numel(k1_list)
    for j = 1:numel(k3_list)
        ...
        x1bout(i,j) = mean(y(end-1000:end,3));
        x2bout(i,j) = mean(y(end-1000:end,6));
        x3bout(i,j) = mean(y(end-1000:end,9));
        x4bout(i,j) = mean(y(end-1000:end,12));
